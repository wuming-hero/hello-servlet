...
<table border="1">
	...
	<tr>
		...
		<td><c:url value="/updateStudent.jsp" var="update">
		...
		<a href="/Jsp/StudentServlet?status=splitPage&currentPage=1">首页</a>
		<c:choose>
			<c:when test="${sessionScope.currentPage==1}">
				上一页
			</c:when>
			<c:otherwise>
				<a href="/Jsp/StudentServlet?status=splitPage&currentPage=${sessionScope.currentPage-1}">上一页</a>
			</c:otherwise>
		</c:choose>
		<c:choose>
			...
			<c:otherwise>
				<a href="/Jsp/StudentServlet?status=splitPage&currentPage=${sessionScope.currentPage+1}">下一页 </a>
			</c:otherwise>
		</c:choose>
		<a href="/Jsp/StudentServlet?status=splitPage&currentPage=${sessionScope.countPage}">尾页 </a>
		<script type="text/javascript">
			function changePage(){
				var currentPage=document.getElementById("currentPage").value;
				window.open("/Jsp/StudentServlet?status=splitPage&currentPage=" + currentPage,"_self");
			}
		</script>
